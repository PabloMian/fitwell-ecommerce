...
const VistaSuplementos = () => {
  ...
  useEffect(() => {
    const fetchProductos = async () => {
      try {
        ...
        setProductos(productosProcesados.filter(p => p.categoria_id === 3));
      } catch (err) {
        console.error(err);
  ...
  if (loading) {
    return (
      <Container className="text-center my-5">
        ...
        <p className="mt-3">Cargando suplementos...</p>
      </Container>
    );
  }
  ...
  return (
    <Container className="mt-4">
      <h2 className="text-center mb-4">Suplementos</h2>
      <Row>
...
export default VistaSuplementos;
